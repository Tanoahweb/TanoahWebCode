...
function renderOrderEmailHtml(order: any, mode: 'admin' | 'customer' = 'admin'): string {
  const orderNum = order.orderNumber || order.order_number || 'TAN-ORDER';
  ...
  const email = order.formData?.email || order.guest_email || order.customer_email || order.customerEmail || 'N/A';
  const phone = order.formData?.phone || order.guest_phone || order.customer_phone || order.customerPhone || 'N/A';
  const items = order.items || [];
  const grandTotal = order.grandTotal ?? order.grand_total ?? 0;
  const subtotal = order.subtotal ?? 0;
  const discount = order.discount ?? order.discount_total ?? 0;
  ...
  const docTitle = isAdmin ? `New Order Alert: ${orderNum}` : `Order Confirmed: ${orderNum}`;
  const subtitle = isAdmin ? 'TANOAH &bull; ADMIN ORDER ALERT' : 'TANOAH &bull; ORDER CONFIRMATION';
  const headline = isAdmin ? `🚨 New Order Received: ${orderNum}` : `✨ Order Confirmed: ${orderNum}`;
  const dateLabel = isAdmin ? `Recorded &bull; ${orderDate}` : `Order Placed &bull; ${orderDate}`;
...
function generateAdminOrderAlertHtml(order: any): string {
  return renderOrderEmailHtml(order, 'admin');
}

function generateCustomerOrderConfirmationHtml(order: any): string {
  return renderOrderEmailHtml(order, 'customer');
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Send Admin Order Alert (using self-contained Screenshot 2 template)
    try {
      const aHtml = adminHtml || generateAdminOrderAlertHtml(order);
      const adminRes = await sendResend({
        from: fromEmail,
        to: adminEmail,
    ...
    // 3. Send Customer Order Confirmation (using self-contained Screenshot 2 template)
    if (customerEmail && customerEmail.includes('@')) {
      const cHtml = customerHtml || generateCustomerOrderConfirmationHtml(order);
