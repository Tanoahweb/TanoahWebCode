...
export const CollectionsPage: React.FC = () => {
  ...
  useEffect(() => {
    loadData();
    window.addEventListener('tanoah_collections_updated', loadData);
    return () => {
      window.removeEventListener('tanoah_collections_updated', loadData);
    };
  }, []);
  ...
  const handleSaveCollection = async (e: React.FormEvent) => {
    ...
    const colToSave: Collection = {
      id: editingCollection ? editingCollection.id : crypto.randomUUID(),
      title: name.trim(),
    ...
    const res = await api.saveCollection(colToSave);
    if (res?.success) {
      addToast({
        type: 'success',
        title: editingCollection ? 'Collection Updated' : 'Collection Created',
        description: `Collection "${colToSave.title}" saved and synced with store navigation.`,
      });
      ...
      addToast({
        type: 'error',
        title: 'Save Failed',
        description: 'Failed to save collection to database.',
      });
    }
  };

  const handleToggleStatus = async (col: Collection) => {
    const newStatus = col.is_active === false ? true : false;
    const updated = { ...col, is_active: newStatus };
    setCollections((prev) => prev.map((c) => (c.id === col.id ? updated : c)));
    const res = await api.saveCollection(updated);
    if (res?.success) {
      addToast({
        type: 'success',
        title: newStatus ? 'Collection Activated' : 'Collection Paused',
        description: `Collection "${col.title}" is now ${newStatus ? 'active' : 'hidden'} on storefront.`,
      });
    } else {
      loadData();
    }
  ...
  const handleDeleteCollection = async (col: Collection) => {
    const matchingProductsCount = products.filter((p) => {
      ...
      return colMatch || tagMatch;
    }).length;

    const confirmMsg =
      matchingProductsCount > 0
        ? `Collection "${col.title}" currently has ${matchingProductsCount} product(s) assigned. Deleting this collection will remove the collection category, but all products will remain safe in your store catalog. Proceed?`
        : `Are you sure you want to remove collection "${col.title}"?`;

    if (window.confirm(confirmMsg)) {
      await api.deleteCollection(col.id, col.slug);
      addToast({
        type: 'info',
                        ...
                        <tr key={col.id || col.slug} className="hover:bg-[#FAFAFA] transition-colors">
                          ...
                          <td className="p-4">
                            <button
                              ...
                              className={`text-[10px] font-semibold px-2.5 py-1 rounded uppercase cursor-pointer transition-colors ${
                                col.is_active !== false
                            ...
                            >
                              {col.is_active !== false ? 'ACTIVE' : 'PAUSED'}
                            </button>
                          </td>
            ...
            <div className="bg-white rounded-[4px] max-w-lg w-full p-6 space-y-4 shadow-xl text-left animate-in fade-in zoom-in-95 duration-150">
              ...
              <form onSubmit={handleSaveCollection} className="space-y-4 text-xs">
                ...
                <div className="flex items-center gap-2 pt-1 pb-1">
                  <input
                    type="checkbox"
                    id="collectionModalIsActive"
                    checked={isActive}
                    onChange={(e) => setIsActive(e.target.checked)}
                    className="w-4 h-4 accent-[#3F3F8F] rounded cursor-pointer"
                  />
                  <label htmlFor="collectionModalIsActive" className="text-xs font-semibold text-black cursor-pointer select-none">
                    Active Collection (Visible on storefront & product assignment)
                  </label>
                </div>
