...
const envFrom = import.meta.env.VITE_RESEND_FROM_EMAIL;
const FROM_EMAIL = (envFrom && !envFrom.includes('resend.dev')) ? envFrom : 'TANOAH <noreply@tanoah.com>';
...
export const generateAdminOrderAlertHtml = (order: OrderEmailPayload): string => {
  ...
  const email = order.formData?.email || (order as any).guest_email || (order as any).customer_email || (order as any).customerEmail || 'N/A';
  const phone = order.formData?.phone || (order as any).guest_phone || (order as any).customer_phone || (order as any).customerPhone || 'N/A';
  const items = order.items || [];
  const grandTotal = order.grandTotal ?? order.grand_total ?? 0;
  const subtotal = order.subtotal ?? 0;
  const discount = order.discount ?? order.discount_total ?? 0;
